...
| 2026-09-15 | The server accepts **three chat model ids the spec's `ChatCompletionLLM` enum does not list**: `Fanar-Agentic`, `Fanar-Sadiq-Agentic`, `Islamic-RAG`. Read verbatim off the validation error for an unknown id, which enumerates the accepted set: `'Fanar', 'Fanar-Agentic', 'Fanar-S-1-7B', 'Fanar-C-1-8.7B', 'Fanar-C-2-27B', 'Islamic-RAG', 'Fanar-Sadiq', 'Fanar-Sadiq-Agentic', 'Fanar-Sadiq-2', 'Fanar-Oryx-IVU-2'`. | `ChatCompletionLLM` lists seven, exactly the SDK's `ChatModel` constants. | `ChatModel.KNOWN` is a subset of what the server takes; `ChatModel.of("Fanar-Agentic")` already works (open value record, ADR-015), so this is a missing-constant gap, not a blocker. All three are **absent from `/v1/models`** for our key (confirmed 2026-09-15) and none has been called, so nothing is known about their behaviour — in particular whether an `-Agentic` variant accepts the user `tools` the base models reject, which is the one question that would change SDK scope (PROJECT_STATE "Deferred"). Shipping constants before calling them would put unverified ids in `KNOWN`. `—` |
...
| Date | Observed | Spec says | Consequence / pinned by |
|---|---|---|---|
| 2026-08-06, re-confirmed 2026-08-29 and 2026-09-15 | The listing is **visibility-scoped per key**, not the callable universe: 11 models listed (byte-identical on all three dates); absent were `Fanar-Sadiq-2`, `Fanar-Diwan`, `Fanar-Sadiq-TTS-1` and `Fanar-Guard-2` — all spec-listed, and Guard-2 is callable by the same key (`LiveModerationsTest` passes). 2026-09-15 widens the gap: the chat endpoint's own validation enum accepts **four** chat ids the listing omits — `Fanar-Sadiq-2` plus the undocumented `Fanar-Agentic`, `Fanar-Sadiq-Agentic` and `Islamic-RAG` ([Chat completions](#chat-completions--post-v1chatcompletions)), so the listing shows 6 of at least 10 callable chat models. Model-level gating hides a model; feature-level gating does not (`Fanar-C-2-27B` stays listed while `enable_thinking` is gated). | 15 models in the rate-limit table; nothing says the listing is scoped. | Never use the listing as an availability oracle (the health indicator reports reachability, not capability). `LiveModelsTest.list_returnsAllKnownChatModels` asserts `KNOWN ⊆ listing` **except** `MODEL_GATED`; a newly gated model joins that set with a dated note, never a weaker assertion. |
...
| 2026-09-15 | **A second, entirely different error shape exists.** Request-validation failures bypass the Fanar envelope and return FastAPI's raw form: `{"detail": [{"type": "enum", "loc": ["body", "model"], "msg": "Input should be 'Fanar', …", "input": "Fanar-Not-A-Model", "ctx": {…}}]}` — no `error` key, so `tryParse` yields `null` and routing falls back to HTTP status (`byStatus`, 422 → `FanarUnprocessableException`). The exception message becomes the entire raw JSON blob. | The spec documents only the `Error` envelope for 4xx. | Two shapes, two routes, both landing on the right exception type — but the validation message is unreadable. Improving it (surface `detail[].loc` / `msg`) is a candidate for its own cycle, listed in PROJECT_STATE. `—` |
