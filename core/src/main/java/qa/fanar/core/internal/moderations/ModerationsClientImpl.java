 ...
 */
public final class ModerationsClientImpl implements ModerationsClient {

    private static final String ENDPOINT = "/v1/moderations";
    private static final String OP_NAME = "fanar.moderations.score";
    ...
    @Override
    public SafetyFilterResponse score(SafetyFilterRequest request) {
    ...
    @Override
    public CompletableFuture<SafetyFilterResponse> scoreAsync(SafetyFilterRequest request) {
        Objects.requireNonNull(request, "request");
        CompletableFuture<SafetyFilterResponse> future = new CompletableFuture<>();
        Thread.ofVirtual().name("fanar-moderations-async-", 0).start(() -> {
            try {
                future.complete(score(request));
            } catch (Throwable t) {
    ...
    private HttpResponse<InputStream> dispatch(SafetyFilterRequest request, ObservationHandle obs) {
    ...
    private byte[] encodeBody(SafetyFilterRequest request) {
        ByteArrayOutputStream buf = new ByteArrayOutputStream();
        try {
            jsonCodec.encode(buf, request);
        } catch (IOException e) {
            throw new FanarTransportException("Failed to encode SafetyFilterRequest", e);
        }
        return buf.toByteArray();
    }

    private SafetyFilterResponse decodeResponse(HttpResponse<InputStream> response) {
        try (InputStream in = response.body()) {
            return jsonCodec.decode(in, SafetyFilterResponse.class);
        } catch (IOException e) {
            throw new FanarTransportException("Failed to decode SafetyFilterResponse", e);
        }
    }
}
