 ...
 */
public final class PoemsClientImpl implements PoemsClient {

    private static final String ENDPOINT = "/v1/poems/generations";
    private static final String OP_NAME = "fanar.poems.generate";
    ...
    @Override
    public PoemGenerationResponse generate(PoemGenerationRequest request) {
    ...
    @Override
    public CompletableFuture<PoemGenerationResponse> generateAsync(PoemGenerationRequest request) {
        Objects.requireNonNull(request, "request");
        CompletableFuture<PoemGenerationResponse> future = new CompletableFuture<>();
        Thread.ofVirtual().name("fanar-poems-async-", 0).start(() -> {
            try {
    ...
    private HttpResponse<InputStream> dispatch(PoemGenerationRequest request, ObservationHandle obs) {
    ...
    private byte[] encodeBody(PoemGenerationRequest request) {
        ByteArrayOutputStream buf = new ByteArrayOutputStream();
        try {
            jsonCodec.encode(buf, request);
        } catch (IOException e) {
            throw new FanarTransportException("Failed to encode PoemGenerationRequest", e);
        }
        return buf.toByteArray();
    }

    private PoemGenerationResponse decodeResponse(HttpResponse<InputStream> response) {
        try (InputStream in = response.body()) {
            return jsonCodec.decode(in, PoemGenerationResponse.class);
        } catch (IOException e) {
            throw new FanarTransportException("Failed to decode PoemGenerationResponse", e);
        }
    }
}
